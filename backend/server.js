...
app.delete('/invItem/:id', async (req, res) => {
  try {
    const results = db.query('DELETE FROM product where id = $1', [req.params.id])
    res.status(204).json({
      status: 'success'
    })
    console.log(results)
...
app.put('/cart/:id', async (req, res) => {
  try {
    const results = await db.query(
      'UPDATE cart SET name = $1, price = $2, description = $3, image = $4, quantity = $5 where id = $6 returning *',
      [req.params.id]
    )
    res.status(200).json({
      status: 'success',
...
app.post('/users', async (req, res) => {
  ...
  try {
    const send = await db.query(
      `INSERT INTO users(id, us, ps, cartInv) values ($1, $2, $3, $4) returning *`,
...
app.post('/signup', validInfo, async (req, res) => {
  ...
  try {
    const user = await db.query('SELECT * FROM users WHERE us = $1', [us])
...
app.post('/invItem', async (req, res) => {
  try {
    const results = await db.query(
      `INSERT INTO product(id, name, price, description, image, quantity, category_id, sku) values ($1, $2, $3, $4, $5, $6, $7, $8) returning *`,
      [
        req.body.id,
        req.body.name,
        req.body.price,
...
app.get('/invItem/show/:id', async (req, res) => {
  console.log(req.body)
  try {
    const results = await db.query(`SELECT * FROM product WHERE id = $1`, [req.params.id])
    console.log(results)
    res.status(200).json({
      status: 'success',
      data: {
